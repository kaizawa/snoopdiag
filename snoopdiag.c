...
int mkbin(){ /* tcp のデータ部より binary file を作る*/ 
	struct connection_t *conn;
	struct stream_t *streams;
...
int view_conn(int optflag){ 
    ...
    int    exp_ack; /* packet が期待する ACK 値 */
    int    self_seq; /* Sefl packet SEQ 値 */
    int    len; /* TCP の data の長さ*/
    double stream_init_time; /* connection 毎の開始時間*/
    double receive_time; /* 個々の packet の到着時間 */
    double previous_time = 0; /* 一つ前の packet の到着時間 */
    ...
    for( ; conn != NULL ; conn = conn->conn_next) {
        ...
        for(streams = conn->stream ; streams != NULL ; streams = streams->stream_next ){
            ...
            else
            {
                ...
                /* 以前の packet に SND_NXT の SEQ が含まれているか調べる & 再送のチェック*/
                /* ここはデータがある全 packet が通るため、大変負荷が高い。要改善         */
                if( len > 0){
            ...
            if(len != 0){ /* データの有無 */
                ...
                /* 続きの packet から期待する ACK があるかどうかを check */
